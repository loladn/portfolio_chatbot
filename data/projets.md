# Projets universitaires de Lola

## Projet web Airbnb Paris

<!-- ### Année de réalisation
2ème année BUT Science des Données -->

### Objectif du projet
Visualisation dynamique des logements Airbnb à Paris avec graphiques interactifs

### Technologies projet Airbnb
- JavaScript
- HTML/CSS
- Graphiques interactifs

### Type de compétences
Informatique, Visualisation de données

---

## Projet accidentologie routière France 2023

<!-- ### Année de réalisation
2ème année BUT -->

### Description du projet
Site web dynamique analysant les accidents corporels en France (2023) via données OpenData

### Fonctionnalités développées
- Graphiques interactifs
- Statistiques descriptives
- Mise à jour de base de données
- Analyses par région, trajet, météo

### Stack technique accidentologie
PHP, MySQL, JavaScript, Highcharts, HTML/CSS

### Compétences mobilisées
Informatique, Bases de données, Visualisation

---

## Étude SIG implantation McDonald's France

<!-- ### Année de réalisation
2ème année BUT -->

### Contexte du projet SIG
Projet réalisé en **autonomie complète** pour maîtriser tous les aspects techniques du SIG

### Objectif
Analyse spatiale de l'implantation des restaurants McDonald's en France

### Outils SIG utilisés
- QGIS
- Données gouvernementales
- Analyse spatiale

### Domaine
Informatique, SIG, Géomatique

---

## Projet Calyxis accidents vie courante

<!-- ### Année de réalisation
1ère année BUT (projet final) -->

### Contexte
Analyse pour **Calyxis**, observatoire des accidents de la vie courante

### Outils développés
- Interface Tkinter pour gestion de données
- Traitements statistiques (Chi-2, analyses bivariées)
- Visualisations Power BI
- Export de données traitées

### Technologies projet Calyxis
Python (Pandas, Tkinter, SciPy), Power BI, Excel

### Méthode de travail
Méthode agile en équipe

### Compétences
Statistiques, Analyse de données, Visualisation

---

## Application VBA recrutement football

<!-- ### Année
2ème année BUT -->

### Objectif application VBA
Automatisation du processus de recrutement pour section sportive football

### Fonctionnalité
- Saisie des performances des candidats
- Classement automatique
- Sélection des meilleurs profils

### Technologies
VBA, Excel

### Type
Informatique, Automatisation

---

## Étude économique Norvège

<!-- ### Année
2ème année BUT -->

### Sujet d'étude
Analyse des importations norvégiennes et prévisions économiques

### Méthodes statistiques utilisées
- Séries temporelles
- Analyse de données OCDE
- Prévisions économiques

### Outil
Excel

### Type
Statistiques, Analyse économique

---

## Projet échantillonnage région PACA

<!-- ### Année
1ère année BUT -->

### Objectif statistique
Étude de population : échantillonnage et estimation pour la région Provence-Alpes-Côte d'Azur

### Méthodes appliquées
- Techniques d'échantillonnage
- Méthodes d'estimation statistique

### Langage
R

### Type
Statistiques

---

## Poster scientifique data visualisation

<!-- ### Année
2ème année BUT -->

### Événement
Fête de la science

### Thème du poster
Data visualisation : présentation des principes et bonnes pratiques

### Compétences
Communication scientifique, Vulgarisation, Design visuel
